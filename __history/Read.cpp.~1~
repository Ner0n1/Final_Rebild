...
__fastcall Read::Read(bool CreateSuspended, AFS* FS, bool Ignore,
sqlite3* DB)
    ...
    for(Iter->first();(!Iter->IsDone());Iter->next())
	 {
		...
		if (sign != -1) {
			...
			if (res != SQLITE_OK)
			{
				Form1->Label1->Text = L"Error on Insert into!";
			}else
			{
				res = sqlite3_step(Stmt);
				if(res != SQLITE_DONE)
				{
						Form1->Label1->Text = L"Error on update table!";
				}
			}
